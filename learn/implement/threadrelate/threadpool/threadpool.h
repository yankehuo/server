		...
		for (int i = 0; i < number && !stop; ++i) {
			...
			if (sockfd == m_listenfd) {
				...
				do {
					// Round Robin
					// why 
					if (m_work_thread[j].m_tid != -1) {
						break;
					}
					j = (j + 1) % m_thread_number;
				...
				// why?
				sub_thread_counter = (j + 1) % m_thread_number;
				...
				if (ret <= 0) {
					...
					for (int j = 0; j < ret && !stop; ++j) {
						switch (signals[j]) {
							case SIGTERM:
							case SIGINT:
							{
								printf("kill all the work thread now\n");
								for (int k = 0; k < m_thread_number; ++k) {
									unsigned long tid = m_work_thread[k].m_tid;
									if (tid != -1) {
										pthread_cancel(tid);
									}
								}
