...
// used_fd store fd
static void addfd(int epollfd, int fd, std::set<int> &used_fd) {
	epoll_event event;
	event.data.fd = fd;
...
static void removefd(int epollfd, int fd,  std::set<int> &used_fd) {
	epoll_ctl(epollfd, EPOLL_CTL_DEL, fd, NULL);
	used_fd.erase(fd);
}

static void close_fd(int epollfd, std::set<int> &used_fd) {
...
void threadpool<T>::worker(void *arg) {
	...
	addfd(m_epollfd, pipefd, used_fd);
		...
		for (int i = 0; i < number; ++i) {
			...
			if ((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
				...
				if (ret <= 0) {
					...
					addfd(m_epollfd, connfd, used_fd);
					// init how to do?
