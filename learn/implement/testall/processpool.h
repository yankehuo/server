...
// m_pid--target PID, m_pipefd--parent and child pipe
class process {
public:
	pid_t m_pid;
	int m_pipefd[2];
...
// added
// forward
template <typename T> void cb_func(T*);
...
template <typename T>
void cb_func(T *user) {
	// epoll_ctl(m_epollfd, EPOLL_CTL_DEL, user->connfd, 0);
	removefd(m_epollfd, user->m_sockfd);
	assert(user);
	close(user->m_sockfd);
}
	...
	while (!m_stop) {
		...
		for (int i = 0; i < number; ++i) {
			...
			if ((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
				...
				if (((ret < 0) && (errno != EAGAIN)) || ret == 0) {
					...
					if(connfd < 0) {
						...
						const char *msg = "sorry! There are many clients!\n";
					...
					// added for timer
					heap_timer<T> *timer = new heap_timer<T>(TIMESLOT);
			...
			// 处理接受的数据
				...
				heap_timer<T> *timer = users[sockfd].timer;
